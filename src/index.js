...
import Service from './utils/ajaxUtil'
...
export default class Timebar {
        ...
        Service.getPeriodList().then((resp) => {
          if (resp.code === 200) {
            ...
            for (let i = 0; i < mockLength; i++) {
              this.philData.push(Mock.mock({
                'importance': Random.pick([1, 1.1, 1.2, 2, 3]),
            ...
            this.nowPhilsLevel = this.philData.filter((item) => {
              return item.importance >= 1 && item.importance < 2
            }).sort((m, n) => m.year - n.year)

            console.log(this.nowPhilsLevel)
            // 输出结果
            this.createCanvas();
            this._resize();
  ...
  // createAvatar(philData) {
  //   this.ctx.save();
  //   this.ctx.scale(this.ratio, this.ratio);
  //   this.ctx.translate(this.translate.x, this.translate.y)

  //   philData.forEach((item) => {
  //    
  //   })
  //   this.ctx.restore();
  // }
  ...
  render() {
    ...
    this.onRender(this);
    // console.log(this.getTimeByPixel(this.$html.height()))
  ...
  drawAvatar() {
    let nowUnitTime = this.unitTime
    let zeroY = this.getTimeByPixel(0);
    let oneScreenTime = this.getTimeByPixel(this.$html.height()) - this.getTimeByPixel(0);

    this.nowPhilsLevel.forEach((phil) => {
      const x = phil.originType === 'EAST' ? this.centerPx + 200 : this.centerPx - 200
      const y = this.getYbyTime(phil.year)
      this.drawCircle(x, y)
      this.drawAvatarLine(x, y)
      // this.drawAvatarText(x, y)
    })
  ...
  drawAvatarLine(x, y) {
    this.ctx.beginPath();
    this.ctx.moveTo(x, y);
    this.ctx.lineTo(this.centerPx, y);
    this.ctx.closePath();
